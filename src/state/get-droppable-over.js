...
function getClosest({
  ...
  draggable,
  candidates,
}: GetClosestArgs): ?DroppableId {
  // By compare the distance between the current draggable center and
  // each candidate's, we get the closest droppable candidate

  const startCenter: Position = pageBorderBox.center;
  const sorted: WithDistance[] = _.orderBy(candidates
    .map((candidate: DroppableDimension): WithDistance => {
      ...
      // smallest value will be first, if distance same, small droppable will be first
...
export default function getDroppableOver({
  ...
  // Going to use fallback: distance from current draggable
  return getClosest({
    pageBorderBox,
    draggable,
    candidates,
